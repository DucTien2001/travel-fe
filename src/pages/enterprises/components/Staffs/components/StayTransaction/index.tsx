...
const Staff = memo(({}: Props) => {
  ...
  const fetchData = (value?: {
    ...
    const params: StatisticRoomBill = {
    ...
    StaffService.statisticRoomBill(params)
      .then((res) => {
        setData({
          data: res.data,
          meta: res.meta,
  ...
  const onRedirectEdit = (item: any) => {
    router.push({
      pathname: `/enterprises/staffs/stay-transaction/${item?.staffId}`,
    });
  };
