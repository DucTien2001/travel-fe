...
import { ITourStatistic } from "models/enterprise/tourBill";
...
const StayStatistic = memo(({}: Props) => {
  ...
  const fetchData = (value?: { take?: number; page?: number }) => {
    ...
    RoomBillService.statisticAll(params)
      .then((res) => {
        if (res.success) {
          setData({
            data: res.data,
  ...
  const onRedirectEdit = (item: IStayStatistic) => {
    router.push({
      pathname: `/enterprises/stayStatistic/${item.stayId}`,
    });
  };
            ...
            <TableBody>
              {data?.data?.length ? (
                data.data?.map((item, index) => {
                  return (
                    <TableRow key={item?.stayId}>
                      <TableCell scope="row" className={classes.tableCell}>
                        {index + 1}
                      </TableCell>
                      <TableCell className={classes.tableCell} component="th">
                        <a
                          href={`/listHotel/:${item?.stayId}`}
                          target="_blank"
                          rel="noreferrer"
